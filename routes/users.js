...
router.get('/', async (req, res) => {
  const sql =
    'SELECT id, sex, firstname, lastname, zipcode, city, country, email, mobile FROM users';
...
router.get('/:id', async (req, res) => {
  const { id } = req.params;
  const sql =
    'SELECT id, sex, firstname, lastname, zipcode, city, country, email, mobile FROM users WHERE id=?';
  const sqlValues = [id];
...
router.put('/:id', async (req, res) => {
  ...
  const { sex, firstname, lastname, zipcode, city, country, email, mobile } =
    req.body;
  const sql =
    'UPDATE users SET sex=?, firstname=?, lastname=?, zipcode=?, city=?, country=?, email=?, mobile=? WHERE id=?';
  const sqlValues = [
    sex,
    firstname,
    lastname,
